...
public class PoetryParserV1 extends AbstractPoetryParser {
    @Override
    protected JsonNode getProjectNode(JsonNode root) {
        return root.at("/tool/poetry");
    }

    @Override
    protected List<UnresolvedModule> extractRuntimeDependencies(JsonNode root) {
        return extractDirectDependencies(root, "/tool/poetry/dependencies");
    }

    @Override
    protected List<UnresolvedModule> extractDevelopmentDependencies(JsonNode root) {
        return extractDirectDependencies(root, "/tool/poetry/group/dev/dependencies");
    }
}
