...
Total Counts: AssessmentReportAdapter.VulnerabilityCounts(criticalCounter=0, highCounter=1, mediumCounter=0, lowCounter=1, noneCounter=2, assessedCounter=4, totalCounter=4)
 - Asset:     asset3 (in Other Assets)
   Counts:    AssessmentReportAdapter.VulnerabilityCounts(criticalCounter=0, highCounter=1, mediumCounter=0, lowCounter=1, noneCounter=2, assessedCounter=4, totalCounter=4)
    ...
    /*@Test
    public void generateCompleteTableTest() throws IOException {
        final Inventory inputInventory = new InventoryReader().readInventory(new File("/Users/ywittmann/workspace/ref/issues/2024-10-25-counting/1/gts-summary-template-report-merged-preserved-contexts.xlsx"));

        final AssessmentReportAdapter adapter = new AssessmentReportAdapter(inputInventory, new CentralSecurityPolicyConfiguration());

        final List<AssessmentReportAdapter.GroupedAssetsVulnerabilityCounts> initial = adapter.groupAssetsByAssetGroup(inputInventory.getAssetMetaData(), false);
        final List<AssessmentReportAdapter.GroupedAssetsVulnerabilityCounts> context = adapter.groupAssetsByAssetGroup(inputInventory.getAssetMetaData(), true);

        log.info("Initial:");
        debugLogAssessmentCounts(initial);
        log.info("");
        log.info("Context:");
        debugLogAssessmentCounts(context);
    }

    @Test
    public void generateSingleRowTest() throws IOException {
        final Inventory inputInventory = new InventoryReader().readInventory(new File("/Users/ywittmann/workspace/ref/issues/2024-10-25-counting/ex-004/report/output.xlsx"));
        final AssetMetaData asset = inputInventory.getAssetMetaData().stream().filter(a -> a.get(AssetMetaData.Attribute.NAME).contains("Training")).findFirst().orElseThrow(() -> new RuntimeException("Desired asset not found."));

        final AssessmentReportAdapter adapter = new AssessmentReportAdapter(inputInventory, new CentralSecurityPolicyConfiguration());

        final AssessmentReportAdapter.VulnerabilityCounts initial = adapter.countVulnerabilities(asset, false);
        log.info(initial.toString());
        log.info("");

        // final AssessmentReportAdapter.VulnerabilityCounts context = adapter.countVulnerabilities(asset, true);
        // log.info(context.toString());
    }

    @Test
    public void mergedContextsTest() throws IOException {
        final Inventory inputInventory = new InventoryReader().readInventory(new File("/Users/ywittmann/workspace/ref/issues/2024-10-25-counting/2/gts-summary-template-report-merged-single-context.xlsx"));
        final AeaaVulnerabilityContextInventory vInventory = AeaaVulnerabilityContextInventory.fromInventory(inputInventory);
        ...
        final StatisticsOverviewTable table = StatisticsOverviewTable.buildTable(securityPolicy, vInventory.getVulnerabilities(), null, false);
        Arrays.asList(table.toString().split("\n")).forEach(log::info);

    }

    private void debugLogAssessmentCounts(List<AssessmentReportAdapter.GroupedAssetsVulnerabilityCounts> grouped) {
        for (AssessmentReportAdapter.GroupedAssetsVulnerabilityCounts group : grouped) {
            group.log();
        }
    }*/
